...
#define DEBUG_TYPE "rbx-isel"
#define PASS_NAME "Rubinius DAG->DAG Pattern Instruction Selection"

// Instruction Selector Implementation
namespace {

class RubiniusDAGToDAGISel : public SelectionDAGISel {

  /// Subtarget - Keep a pointer to the RubiniusSubtarget around so that we can
  /// make the right decision when generating code for different subtargets.
  const RubiniusSubtarget *Subtarget;

public:
  RubiniusDAGToDAGISel() = delete;

  explicit RubiniusDAGToDAGISel(RubiniusTargetMachine &TM)
      : SelectionDAGISel(TM), Subtarget(nullptr) {}

  bool runOnMachineFunction(MachineFunction &MF) override {
    // Reset the subtarget each time through.
    Subtarget = &MF.getSubtarget<RubiniusSubtarget>();
...
#include "RubiniusGenDAGISel.inc"
...
class RubiniusDAGToDAGISelLegacy : public SelectionDAGISelLegacy {
public:
  static char ID;
  RubiniusDAGToDAGISelLegacy(RubiniusTargetMachine &TM)
      : SelectionDAGISelLegacy(ID, std::make_unique<RubiniusDAGToDAGISel>(TM)) {}
};
} // namespace

char RubiniusDAGToDAGISelLegacy::ID = 0;

// ComplexPattern used on Rubinius Load/Store instructions
bool RubiniusDAGToDAGISel::SelectAddr(SDValue Addr, SDValue &Base, SDValue &Offset) {
...
// ComplexPattern used on Rubinius FI instruction
bool RubiniusDAGToDAGISel::SelectFIAddr(SDValue Addr, SDValue &Base,
                                   SDValue &Offset) {
  SDLoc DL(Addr);
...
bool RubiniusDAGToDAGISel::SelectInlineAsmMemoryOperand(
...
void RubiniusDAGToDAGISel::Select(SDNode *Node) {
...
void RubiniusDAGToDAGISel::PreprocessLoad(SDNode *Node,
...
void RubiniusDAGToDAGISel::PreprocessISelDAG() {
...
bool RubiniusDAGToDAGISel::getConstantFieldValue(const GlobalAddressSDNode *Node,
...
bool RubiniusDAGToDAGISel::fillGenericConstant(const DataLayout &DL,
                                          const Constant *CV,
                                          val_vec_type &Vals, uint64_t Offset) {
...
bool RubiniusDAGToDAGISel::fillConstantDataArray(const DataLayout &DL,
...
bool RubiniusDAGToDAGISel::fillConstantArray(const DataLayout &DL,
...
bool RubiniusDAGToDAGISel::fillConstantStruct(const DataLayout &DL,
...
void RubiniusDAGToDAGISel::PreprocessTrunc(SDNode *Node,
                                      SelectionDAG::allnodes_iterator &I) {
...
FunctionPass *llvm::createRubiniusISelDag(RubiniusTargetMachine &TM) {
  return new RubiniusDAGToDAGISelLegacy(TM);
}
