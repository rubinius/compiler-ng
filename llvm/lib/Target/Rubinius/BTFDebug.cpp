...
void BTFTypeDerived::completeType(BTFDebug &BDebug) {
  ...
  case BTF::BTF_KIND_RESTRICT:
    ...
    //
    // Furthermore, the Linux kernel refuses to load Rubinius programs that contain
...
void BTFDebug::processGlobalValue(const MachineOperand &MO) {
  ...
  if (MO.isGlobal()) {
    ...
    if (!GVar->hasAttribute(RubiniusCoreSharedInfo::AmaAttr) &&
        !GVar->hasAttribute(RubiniusCoreSharedInfo::TypeIdAttr))
      return;
    ...
    generatePatchImmReloc(ORSym, RootId, GVar,
                          GVar->hasAttribute(RubiniusCoreSharedInfo::AmaAttr));
  }
}
...
void BTFDebug::beginInstruction(const MachineInstr *MI) {
  ...
  if (MI->getOpcode() == Rubinius::LD_imm64) {
  ...
  } else if (MI->getOpcode() == Rubinius::CORE_LD64 ||
             MI->getOpcode() == Rubinius::CORE_LD32 ||
             MI->getOpcode() == Rubinius::CORE_ST ||
             MI->getOpcode() == Rubinius::CORE_SHIFT) {
    // relocation insn is a load, store or shift insn.
    processGlobalValue(MI->getOperand(3));
  } else if (MI->getOpcode() == Rubinius::JAL) {
...
/// initialization of Rubinius maps.
///
/// struct {
///   __uint(type, Rubinius_MAP_TYPE_PROG_ARRAY);
///   ...
...
bool BTFDebug::InstLower(const MachineInstr *MI, MCInst &OutMI) {
  if (MI->getOpcode() == Rubinius::LD_imm64) {
    ...
    if (MO.isGlobal()) {
      ...
      if (GVar) {
        ...
        if (Reloc == BTF::ENUM_VALUE_EXISTENCE || Reloc == BTF::ENUM_VALUE ||
          ...
          OutMI.setOpcode(Rubinius::LD_imm64);
        else
          OutMI.setOpcode(Rubinius::MOV_ri);
    ...
    if (MO.isGlobal()) {
      const GlobalValue *GVal = MO.getGlobal();
      auto *GVar = dyn_cast<GlobalVariable>(GVal);
      if (GVar && GVar->hasAttribute(RubiniusCoreSharedInfo::AmaAttr)) {
...
void BTFDebug::endModule() {
  ...
  // In case that Rubinius_TRAP usage is removed during machine-level optimization,
  // generate btf for Rubinius_TRAP function here.
  for (const Function &F : *MMI->getModule()) {
    if (F.getName() == Rubinius_TRAP)
