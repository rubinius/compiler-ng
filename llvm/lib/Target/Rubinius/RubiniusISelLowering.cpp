...
#define DEBUG_TYPE "rbx-lower"

static cl::opt<bool> RubiniusExpandMemcpyInOrder("rbx-expand-memcpy-in-order",
  cl::Hidden, cl::init(false),
  cl::desc("Expand memcpy into load/store pairs in order"));

static cl::opt<unsigned> RubiniusMinimumJumpTableEntries(
    "rbx-min-jump-table-entries", cl::init(13), cl::Hidden,
    cl::desc("Set minimum number of entries to use a jump table on Rubinius"));
...
RubiniusTargetLowering::RubiniusTargetLowering(const TargetMachine &TM,
  ...
  addRegisterClass(MVT::i64, &Rubinius::GPRRegClass);
  if (STI.getHasAlu32())
    addRegisterClass(MVT::i32, &Rubinius::GPR32RegClass);

  // Compute derived properties from the register classes
  computeRegisterProperties(STI.getRegisterInfo());

  setStackPointerRegisterToSaveRestore(Rubinius::R11);
  ...
  setMinimumJumpTableEntries(RubiniusMinimumJumpTableEntries);

  // Function alignments
  setMinFunctionAlignment(Align(8));
  setPrefFunctionAlignment(Align(8));

  if (RubiniusExpandMemcpyInOrder) {
    ...
    // will cause trouble to memcpy pattern matcher inside kernel eRubinius JIT
    // compilers.
    //
...
bool RubiniusTargetLowering::allowsMisalignedMemoryAccesses(EVT VT, unsigned, Align,
...
bool RubiniusTargetLowering::isOffsetFoldingLegal(const GlobalAddressSDNode *GA) const {
  return false;
}

bool RubiniusTargetLowering::isTruncateFree(Type *Ty1, Type *Ty2) const {
...
bool RubiniusTargetLowering::isTruncateFree(EVT VT1, EVT VT2) const {
...
bool RubiniusTargetLowering::isZExtFree(Type *Ty1, Type *Ty2) const {
...
bool RubiniusTargetLowering::isZExtFree(EVT VT1, EVT VT2) const {
...
bool RubiniusTargetLowering::isZExtFree(SDValue Val, EVT VT2) const {
...
unsigned RubiniusTargetLowering::getJumpTableEncoding() const {
  return MachineJumpTableInfo::EK_BlockAddress;
}

RubiniusTargetLowering::ConstraintType
RubiniusTargetLowering::getConstraintType(StringRef Constraint) const {
...
RubiniusTargetLowering::getRegForInlineAsmConstraint(const TargetRegisterInfo *TRI,
  ...
  if (Constraint.size() == 1) {
    ...
    switch (Constraint[0]) {
    case 'r': // GENERAL_REGS
      return std::make_pair(0U, &Rubinius::GPRRegClass);
    case 'w':
      if (HasAlu32)
        return std::make_pair(0U, &Rubinius::GPR32RegClass);
      break;
    default:
      break;
    }
...
void RubiniusTargetLowering::ReplaceNodeResults(
...
SDValue RubiniusTargetLowering::LowerOperation(SDValue Op, SelectionDAG &DAG) const {
  switch (Op.getOpcode()) {
  default:
...
#include "RubiniusGenCallingConv.inc"

SDValue RubiniusTargetLowering::LowerFormalArguments(
  ...
  CCInfo.AnalyzeFormalArguments(Ins, getHasAlu32() ? CC_Rubinius32 : CC_Rubinius64);
    ...
    if (VA.isRegLoc()) {
      ...
      case MVT::i64:
        Register VReg = RegInfo.createVirtualRegister(
            SimpleTy == MVT::i64 ? &Rubinius::GPRRegClass : &Rubinius::GPR32RegClass);
...
const size_t RubiniusTargetLowering::MaxArgs = 5;
...
SDValue RubiniusTargetLowering::LowerCall(TargetLowering::CallLoweringInfo &CLI,
  ...
  // Rubinius target does not support tail call optimization.
  IsTailCall = false;
  ...
  CCInfo.AnalyzeCallOperands(Outs, getHasAlu32() ? CC_Rubinius32 : CC_Rubinius64);
  ...
  if (GlobalAddressSDNode *G = dyn_cast<GlobalAddressSDNode>(Callee)) {
    ...
    if (StringRef(E->getSymbol()) != Rubinius_TRAP) {
  ...
  if (HasFastCall) {
    ...
    if (!CLI.CB->getType()->isVoidTy())
      resetRegMaskBit(TRI, RegMask, Rubinius::R0);
  ...
  Chain = DAG.getNode(RubiniusISD::CALL, CLI.DL, NodeTys, Ops);
  InGlue = Chain.getValue(1);
...
SDValue
RubiniusTargetLowering::LowerReturn(SDValue Chain, CallingConv::ID CallConv,
                               bool IsVarArg,
                               const SmallVectorImpl<ISD::OutputArg> &Outs,
                               const SmallVectorImpl<SDValue> &OutVals,
                               const SDLoc &DL, SelectionDAG &DAG) const {
  unsigned Opc = RubiniusISD::RET_GLUE;
  ...
  CCInfo.AnalyzeReturn(Outs, getHasAlu32() ? RetCC_Rubinius32 : RetCC_Rubinius64);
...
SDValue RubiniusTargetLowering::LowerCallResult(
  ...
  CCInfo.AnalyzeCallResult(Ins, getHasAlu32() ? RetCC_Rubinius32 : RetCC_Rubinius64);
...
SDValue RubiniusTargetLowering::LowerSDIVSREM(SDValue Op, SelectionDAG &DAG) const {
  SDLoc DL(Op);
  fail(DL, DAG,
...
SDValue RubiniusTargetLowering::LowerDYNAMIC_STACKALLOC(SDValue Op,
...
SDValue RubiniusTargetLowering::LowerBR_CC(SDValue Op, SelectionDAG &DAG) const {
  ...
  return DAG.getNode(RubiniusISD::BR_CC, DL, Op.getValueType(), Chain, LHS, RHS,
                     DAG.getConstant(CC, DL, LHS.getValueType()), Dest);
}

SDValue RubiniusTargetLowering::LowerSELECT_CC(SDValue Op, SelectionDAG &DAG) const {
  ...
  return DAG.getNode(RubiniusISD::SELECT_CC, DL, Op.getValueType(), Ops);
}

SDValue RubiniusTargetLowering::LowerATOMIC_LOAD_STORE(SDValue Op,
                                                  SelectionDAG &DAG) const {
  SDNode *N = Op.getNode();
  SDLoc DL(N);
...
static Function *createRubiniusUnreachable(Module *M) {
  if (auto *Fn = M->getFunction(Rubinius_TRAP))
    return Fn;

  FunctionType *FT = FunctionType::get(Type::getVoidTy(M->getContext()), false);
  Function *NewF =
      Function::Create(FT, GlobalValue::ExternalWeakLinkage, Rubinius_TRAP, M);
  ...
  DISubprogram *SP =
      DBuilder.createFunction(CU, Rubinius_TRAP, Rubinius_TRAP, nullptr, 0, FuncType, 0,
                              DINode::FlagZero, DISubprogram::SPFlagZero);
  NewF->setSubprogram(SP);
  return NewF;
}

SDValue RubiniusTargetLowering::LowerTRAP(SDValue Op, SelectionDAG &DAG) const {
  MachineFunction &MF = DAG.getMachineFunction();
  ...
  Function *Fn = createRubiniusUnreachable(MF.getFunction().getParent());
  auto PtrVT = getPointerTy(MF.getDataLayout());
...
SDValue RubiniusTargetLowering::LowerJumpTable(SDValue Op, SelectionDAG &DAG) const {
...
SDValue RubiniusTargetLowering::getAddr(NodeTy *N, SelectionDAG &DAG,
                                   unsigned Flags) const {
  SDLoc DL(N);

  SDValue GA = getTargetNode(N, DL, MVT::i64, DAG, Flags);

  return DAG.getNode(RubiniusISD::Wrapper, DL, MVT::i64, GA);
}

SDValue RubiniusTargetLowering::LowerGlobalAddress(SDValue Op,
                                              SelectionDAG &DAG) const {
  ...
  return SDValue(DAG.getMachineNode(Rubinius::LDIMM64, DL, MVT::i64, Addr), 0);
}

SDValue RubiniusTargetLowering::LowerConstantPool(SDValue Op,
                                             SelectionDAG &DAG) const {
...
SDValue RubiniusTargetLowering::LowerBlockAddress(SDValue Op,
                                             SelectionDAG &DAG) const {
...
unsigned
RubiniusTargetLowering::EmitSubregExt(MachineInstr &MI, MachineBasicBlock *BB,
                                 unsigned Reg, bool isSigned) const {
  const TargetInstrInfo &TII = *BB->getParent()->getSubtarget().getInstrInfo();
  const TargetRegisterClass *RC = getRegClassFor(MVT::i64);
  int RShiftOp = isSigned ? Rubinius::SRA_ri : Rubinius::SRL_ri;
  ...
  if (!isSigned) {
    ...
    BuildMI(BB, DL, TII.get(Rubinius::MOV_32_64), PromotedReg0).addReg(Reg);
    return PromotedReg0;
  }
  Register PromotedReg0 = RegInfo.createVirtualRegister(RC);
  ...
  if (HasMovsx) {
    BuildMI(BB, DL, TII.get(Rubinius::MOVSX_rr_32), PromotedReg0).addReg(Reg);
  } else {
    BuildMI(BB, DL, TII.get(Rubinius::MOV_32_64), PromotedReg0).addReg(Reg);
    BuildMI(BB, DL, TII.get(Rubinius::SLL_ri), PromotedReg1)
...
MachineBasicBlock *
RubiniusTargetLowering::EmitInstrWithCustomInserterMemcpy(MachineInstr &MI,
  ...
  // This function does custom insertion during lowering RubiniusISD::MEMCPY which
  // only has two register operands from memcpy semantics, the copy source
  // address and the copy destination address.
  //
  // Because we will expand RubiniusISD::MEMCPY into load/store pairs, we will need
  ...
  ScratchReg = MRI.createVirtualRegister(&Rubinius::GPRRegClass);
...
MachineBasicBlock *RubiniusTargetLowering::EmitInstrWithCustomInserterLDimm64(
    MachineInstr &MI, MachineBasicBlock *BB) const {
  MachineFunction *MF = BB->getParent();
  const RubiniusInstrInfo *TII = MF->getSubtarget<RubiniusSubtarget>().getInstrInfo();
  const TargetRegisterClass *RC = getRegClassFor(MVT::i64);
  MachineRegisterInfo &RegInfo = MF->getRegInfo();
  DebugLoc DL = MI.getDebugLoc();
  ...
  if (MO.isBlockAddress()) {
    ...
    BuildMI(*BB, MI, DL, TII->get(Rubinius::LD_imm64), TmpReg)
        .addJumpTableIndex(JTI);
    BuildMI(*BB, MI, DL, TII->get(Rubinius::LDD), ResultReg)
        .addReg(TmpReg)
        .addImm(0);
    MI.eraseFromParent();
    return BB;
  ...
  auto emitLDImm64 = [&](const GlobalValue *GV = nullptr, unsigned JTI = -1) {
    auto MIB = BuildMI(*BB, MI, DL, TII->get(Rubinius::LD_imm64), ResultReg);
    if (GV)
...
MachineBasicBlock *
RubiniusTargetLowering::EmitInstrWithCustomInserter(MachineInstr &MI,
                                               MachineBasicBlock *BB) const {
  const TargetInstrInfo &TII = *BB->getParent()->getSubtarget().getInstrInfo();
  DebugLoc DL = MI.getDebugLoc();
  unsigned Opc = MI.getOpcode();
  bool isSelectRROp = (Opc == Rubinius::Select ||
                       Opc == Rubinius::Select_64_32 ||
                       Opc == Rubinius::Select_32 ||
                       Opc == Rubinius::Select_32_64);

  bool isMemcpyOp = Opc == Rubinius::MEMCPY;
  bool isLDimm64Op = Opc == Rubinius::LDIMM64;

#ifndef NDEBUG
  bool isSelectRIOp = (Opc == Rubinius::Select_Ri ||
                       Opc == Rubinius::Select_Ri_64_32 ||
                       Opc == Rubinius::Select_Ri_32 ||
                       Opc == Rubinius::Select_Ri_32_64);
  ...
  bool is32BitCmp = (Opc == Rubinius::Select_32 ||
                     Opc == Rubinius::Select_32_64 ||
                     Opc == Rubinius::Select_Ri_32 ||
                     Opc == Rubinius::Select_Ri_32_64);
...
#define SET_NEWCC(X, Y) \
  case ISD::X: \
    if (is32BitCmp && HasJmp32) \
      NewCC = isSelectRROp ? Rubinius::Y##_rr_32 : Rubinius::Y##_ri_32; \
    else \
      NewCC = isSelectRROp ? Rubinius::Y##_rr : Rubinius::Y##_ri; \
    break
  ...
  // eRubinius at the moment only has 64-bit comparison. Any 32-bit comparison need
  ...
  // try to remove those unnecessary in RubiniusMIPeephole pass.
  if (is32BitCmp && !HasJmp32)
  ...
  // ...
  BB = Copy1MBB;
  BuildMI(*BB, BB->begin(), DL, TII.get(Rubinius::PHI), MI.getOperand(0).getReg())
...
EVT RubiniusTargetLowering::getSetCCResultType(const DataLayout &, LLVMContext &,
                                          EVT VT) const {
  return getHasAlu32() ? MVT::i32 : MVT::i64;
}

MVT RubiniusTargetLowering::getScalarShiftAmountTy(const DataLayout &DL,
                                              EVT VT) const {
  return (getHasAlu32() && VT == MVT::i32) ? MVT::i32 : MVT::i64;
}

bool RubiniusTargetLowering::isLegalAddressingMode(const DataLayout &DL,
...
bool RubiniusTargetLowering::shouldSignExtendTypeInLibCall(Type *Ty,
                                                      bool IsSigned) const {
  return IsSigned || Ty->isIntegerTy(32);
}

bool RubiniusTargetLowering::CanLowerReturn(
  ...
  return CCInfo.CheckReturn(Outs, getHasAlu32() ? RetCC_Rubinius32 : RetCC_Rubinius64);
}
